...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GMCRootMotionModifier)
...
APawn* URootMotionModifier::GetCharacterOwner() const
{
...
{
	const APawn* CharacterOwner = GetCharacterOwner();
...
void URootMotionModifier::OnStateChanged(ERootMotionModifierState LastState)
{
	if (UMotionWarpingComponent* OwnerComp = GetOwnerComponent())
	{
		...
		{
			APawn* CharacterOwner = OwnerComp->GetCharacterOwner();
...
void URootMotionModifier_Warp::Update(const FMotionWarpingUpdateContext& Context)
{
	...
	if (OwnerComp && GetState() == ERootMotionModifierState::Active)
	{
		...
		if (WarpPointAnimProvider != EWarpPointAnimProvider::None)
		{
			...
			{
				if (APawn* CharacterOwner = GetCharacterOwner())
				{
...
FQuat URootMotionModifier_Warp::GetTargetRotation() const
{
	...
	{
		if (const APawn* CharacterOwner = GetCharacterOwner())
		{
...
FQuat URootMotionModifier_Warp::WarpRotation(const FTransform& RootMotionDelta, const FTransform& RootMotionTotal, float DeltaSeconds)
{
	...
	IGMCMotionWarpInterface* MotionWarpInterface =Cast<IGMCMotionWarpInterface>(CharacterOwner);
	if (CharacterOwner == nullptr)
	{
		return FQuat::Identity;
	}
	...
	const FQuat CurrentRotation = CharacterOwner->GetActorQuat() * MotionWarpInterface->GetRotationOffset();
	const FQuat TargetRotation = CurrentRotation.Inverse() * (GetTargetRotation() * MotionWarpInterface->GetRotationOffset());
...
void URootMotionModifier_Warp::PrintLog(const FString& Name, const FTransform& OriginalRootMotion, const FTransform& WarpedRootMotion) const
{
	if (APawn* CharacterOwner = GetCharacterOwner())
	{
		...
		const FVector FutureLocation = CurrentLocation + (MotionWarpInterfacePawn->GetMotionWarpMesh()->ConvertLocalRootMotionToWorld(WarpedRootMotion)).GetTranslation();
...
FTransform UDEPRECATED_RootMotionModifier_SimpleWarp::ProcessRootMotion(const FTransform& InRootMotion, float DeltaSeconds)
{
	...
	if (bWarpTranslation)
	{
		...
		const FTransform MeshRelativeTransform = FTransform(MotionWarpInterfacePawn->GetRotationOffset(), MotionWarpInterfacePawn->GetTranslationOffset());
