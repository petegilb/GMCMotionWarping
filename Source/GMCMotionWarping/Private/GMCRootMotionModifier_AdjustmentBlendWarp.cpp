...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GMCRootMotionModifier_AdjustmentBlendWarp)
...
void URootMotionModifier_AdjustmentBlendWarp::OnTargetTransformChanged()
{
	...
	const USkeletalMeshComponent* SkelMeshComp = CharacterOwner ? MotionWarpInterfacePawn->GetMotionWarpMesh() : nullptr;
...
void URootMotionModifier_AdjustmentBlendWarp::PrecomputeWarpedTracks()
{
	...
	IGMCMotionWarpInterface* MotionWarpInterfacePawn = Cast<IGMCMotionWarpInterface>(CharacterOwner);
	const FBoneContainer& BoneContainer = MotionWarpInterfacePawn->GetMotionWarpMesh()->GetAnimInstance()->GetRequiredBones();
...
void URootMotionModifier_AdjustmentBlendWarp::DrawDebugWarpedTracks(float DrawDuration) const
{
	...
	if (World && Result.GetNum() > 0 && PreviousPosition <= EndTime)
	{
		...
		IGMCMotionWarpInterface* MotionWarpInterfacePawn = Cast<IGMCMotionWarpInterface>(CharacterOwner);
		const FTransform& MeshTransform = MotionWarpInterfacePawn->GetMotionWarpMesh()->GetComponentTransform();
...
void URootMotionModifier_AdjustmentBlendWarp::GetAdjustmentBlendIKBoneTransformAndAlpha(APawn* Character, FName BoneName, FTransform& OutTransform, float& OutAlpha)
{
	OutTransform = FTransform::Identity;
	OutAlpha = 0.f;

	IGMCMotionWarpInterface* MotionWarpInterfacePawn = Cast<IGMCMotionWarpInterface>(Character);
		...
		for (URootMotionModifier* Modifier : MotionWarpingComp->GetModifiers())
		{
			if (Modifier && Modifier->GetState() == ERootMotionModifierState::Active)
			{
				if (URootMotionModifier_AdjustmentBlendWarp* AdjustmentBlendWarpModifier = Cast<URootMotionModifier_AdjustmentBlendWarp>(Modifier))
				{
					...
					const FAnimMontageInstance* RootMotionMontageInstance = MotionWarpInterfacePawn->GetRootMotionAnimMontageInstance(MotionWarpInterfacePawn->GetMotionWarpMesh());
